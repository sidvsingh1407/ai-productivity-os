...
class Audit(Base):
    ...
    form_response: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    ...
    compliance_risk_reasons: Mapped[list | dict | None] = mapped_column(JSONB, nullable=True)
...
class AuditVersion(Base):
    ...
    scores_snapshot: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
